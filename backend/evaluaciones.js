...
// Agregar columna comentario si no existe (migración automática)
pool.query(`ALTER TABLE evaluaciones ADD COLUMN IF NOT EXISTS comentario TEXT`).catch(() => {});

// GET evaluaciones individuales por evaluador de un árbitro en un evento
...
// GET todas las evaluaciones individuales de un evento (para vista detallada admin)
router.get('/detalle/evento/:eventoId', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT
         e.*,
         a.nombre as arbitro_nombre, a.provincia, a.licencia, a.fepaka_id,
         u.nombre as evaluador_nombre,
         ROUND(((e.conformidad+e.manejo_tatami+e.instrucciones+e.aplicacion_reglamento+e.presencia)/5)::numeric, 2) as promedio_evaluador
       FROM evaluaciones e
...
// POST guardar evaluación con comentario (crea o actualiza)
router.post('/', authMiddleware, async (req, res) => {
  try {
    const { evento_id, arbitro_id, conformidad, manejo_tatami, instrucciones, aplicacion_reglamento, presencia, comentario } = req.body;
    const result = await pool.query(
      `INSERT INTO evaluaciones (evento_id, arbitro_id, usuario_id, conformidad, manejo_tatami, instrucciones, aplicacion_reglamento, presencia, comentario)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       ON CONFLICT (evento_id, arbitro_id, usuario_id)
       DO UPDATE SET conformidad=$4, manejo_tatami=$5, instrucciones=$6, aplicacion_reglamento=$7, presencia=$8, comentario=$9, created_at=NOW()
       RETURNING *`,
      [evento_id, arbitro_id, req.user.id, conformidad, manejo_tatami, instrucciones, aplicacion_reglamento, presencia, comentario || null]
    );
