...
pool.query(`ALTER TABLE arbitros ADD COLUMN IF NOT EXISTS apellido VARCHAR(80)`).catch(() => {});

// Helper SQL para nombre completo
...
// GET todos los árbitros (base global + legacy con evento_id)
...
router.post('/', authMiddleware, adminOnly, upload.single('foto'), async (req, res) => {
  try {
    ...
    if (fepaka_id && fepaka_id.trim()) {
      const exists = await pool.query(
        `SELECT id, nombre, COALESCE(apellido,'') as apellido
         FROM arbitros WHERE fepaka_id = $1`,
        [fepaka_id.trim()]
      );
    ...
    const result = await pool.query(
      ...
      [nombre?.trim(), apellido?.trim() || null,
       provincia, club, fepaka_id?.trim() || null, licencia, foto_url]
    );
    ...
    for (const a of arbitros) {
      try {
        ...
        const r = await pool.query(
          ...
          [nombre, apellido, a.provincia, a.club,
           a.fepaka_id || null, a.licencia]
        );
